...
public class Ground : MonoBehaviour
{
    public bool GroundCollision(Vector3 fallingObjectPos, float objectHeight)   // Part2: Medthod activates from all 3 shape collision scripts. 
    {
        ...
        float radius = objectHeight / 2;                                        // Part2: Declares float to hold the falling Objects radius by taking its hieght and dividing by 2.
        ...
        float distanceToGround = groundY - (fallingObjectPos.y - radius);       // Part2: Declares float to hold variable for remaining distance to the ground from falling objects radius.
        if (fallingObjectPos.y - radius <= groundY)                             // Part2: IF falling objects radius is less or greater then our ground objects Y axis... 
        {
